...
var exposed_identity_ids: Array[String] = []
...
func advance_chapter() -> bool:
	...
	var saved_exposed := exposed_identity_ids.duplicate()
	...
	exposed_identity_ids = saved_exposed
...
func add_intel_testimony(question_id: String, option_id: String, npc: Dictionary, trusted: bool) -> bool:
	...
	var source_id := "%d:%s" % [chapter_index + 1, String(npc.get("id", npc.get("public_name", "NPC")))]
	...
	sources.append({
		...
		"npc_name": String(npc.get("public_name", "NPC")),
...
func begin_current_dialogue() -> void:
	...
	var npc := current_npc()
	if not npc.is_empty():
		var text := "第 %d 回合，开始与 %s 对话。" % [chapter_round + 1, npc.get("public_name", "未知 NPC")]
...
func add_dialogue(role: String, content: String) -> void:
	...
	full_dialogue_history.append({
		...
		"npc_name": current_npc().get("public_name", "NPC"),
		"role": role,
		"content": content
	})
	var npc := current_npc()
	if npc.is_empty():
		return
	var speaker := "玩家" if role == "player" else String(npc.get("public_name", "NPC"))
	var listener := String(npc.get("public_name", "NPC")) if role == "player" else "玩家"
...
func format_history() -> String:
	...
	for item in dialogue_history:
		var role := "玩家角色" if item.get("role") == "player" else "NPC"
...
func format_full_history() -> String:
	...
	for item in full_dialogue_history:
		var role := "你方" if item.get("role") == "player" else "NPC"
		lines.append("[第%s回合 %s] %s：%s" % [item.get("round", 0), item.get("npc_name", "NPC"), role, item.get("content", "")])
	return "\n".join(lines)
...
func public_snapshot() -> Dictionary:
	return {
		...
		"exposed_identity_ids": exposed_identity_ids,
